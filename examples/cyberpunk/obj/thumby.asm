...
; This file was generated Thu Jun 22 01:40:05 2017
;--------------------------------------------------------
	.module thumby
	.optsdcc -mgbz80
...
;thumby.c:324: unsigned char alphabet[] =
	...
	xor	a, a
	ld	(bc),a
;thumby.c:428: unsigned char arrow_tile[] =
	ld	hl,#_arrow_tile
	ld	(hl),#0x00
	ld	bc,#_arrow_tile + 1
	xor	a, a
	...
	xor	a, a
	ld	(bc),a
;thumby.c:438: unsigned char text_map[] =
	ld	hl,#_text_map
	ld	(hl),#0x16
	...
	ld	(hl),#0x04
;thumby.c:445: unsigned char python_puzzle[] =
...
;--------------------------------------------------------
	.area _CODE
;thumby.c:457: void set_text_map(char *text, UBYTE len)
;	---------------------------------
...
_set_text_map:
	add	sp, #-5
;thumby.c:466: memset(text_map, 4, text_map_width*text_map_height);
	ld	de,#_text_map
	ld	hl,#0x0020
	push	hl
	ld	l, #0x04
	...
	call	_memset
	add	sp, #6
;thumby.c:469: if (!text || len > 32 || len <= 0) {
	ldhl	sp,#8
	ld	a,(hl-)
	or	a,(hl)
	jp	Z,00129$
...
00127$:
	...
	sub	a, (hl)
	jp	NC,00125$
;thumby.c:478: if (text[i] >= 'A' && text[i] <= 'Z') {
	dec	hl
	dec	hl
	ld	e,(hl)
	inc	hl
	...
	ld	a,(de)
	ld	c,a
;thumby.c:479: text_map[i] = text[i] - 'A' + 0xF; // eg. character A - ascii value of A + index of A in VRAM
	ld	de,#_text_map
	ldhl	sp,#0
	ld	l,(hl)
	ld	h,#0x00
...
;thumby.c:479: text_map[i] = text[i] - 'A' + 0xF; // eg. character A - ascii value of A + index of A in VRAM
	...
	ld	(hl),b
00106$:
;thumby.c:483: if (text[i] >= '0' && text[i] <= '9') {
	ldhl	sp,#4
	dec	hl
	ld	e,(hl)
	inc	hl
...
00174$:
	jr	C,00109$
;thumby.c:484: text_map[i] = text[i] - '0' + 0x5;
	ld	a,b
	add	a, #0xD5
	ld	b,a
	ldhl	sp,#1
...
;thumby.c:493: text_map[i] = 0x2B;
	...
	jr	00128$
00117$:
;thumby.c:494: } else if (text[i] == '?') {
	ld	a,b
	sub	a, #0x3F
	jr	NZ,00114$
;thumby.c:495: text_map[i] = 0x2C;
	ldhl	sp,#1
	ld	a,(hl+)
	ld	h,(hl)
	ld	l,a
	...
	jr	00128$
00114$:
;thumby.c:496: } else if (text[i] == '~') {
	ld	a,b
	sub	a, #0x7E
	jr	NZ,00128$
;thumby.c:497: text_map[i] = 0x2D;
	ldhl	sp,#1
	ld	a,(hl+)
	ld	h,(hl)
	ld	l,a
...
;thumby.c:476: for (i = 0; i < len; ++i) {
	...
	jp	00127$
00125$:
;thumby.c:502: text_map[len] = arrow_address_1;
	ld	de,#_text_map
	ldhl	sp,#9
	ld	l,(hl)
	ld	h,#0x00
...
00129$:
	...
	ret
_set_text_map_end::
;thumby.c:507: void clear_window()
;	---------------------------------
; Function clear_window
; ---------------------------------
_clear_window_start::
_clear_window:
;thumby.c:510: memset(text_map, 4, text_map_width*text_map_height);
	ld	de,#_text_map
	ld	hl,#0x0020
	push	hl
	ld	l, #0x04
	...
	call	_memset
	add	sp, #6
;thumby.c:511: set_win_tiles(2, 2, text_map_width, text_map_height, text_map);
	ld	de,#_text_map
	push	de
	ld	hl,#0x0210
	push	hl
	...
	ret
_clear_window_end::
;thumby.c:514: UBYTE divide_ubyte(UBYTE a, UBYTE b)
;	---------------------------------
; Function divide_ubyte
; ---------------------------------
_divide_ubyte_start::
_divide_ubyte:
;thumby.c:518: while (a >= b) {
	ld	b,#0x00
00101$:
	ldhl	sp,#2
	ld	a,(hl+)
...
;thumby.c:520: result++;
	...
	jr	00101$
00103$:
;thumby.c:522: return result;
	ld	e,b
	ret
_divide_ubyte_end::
;thumby.c:530: void scroll_text(UBYTE len)
;	---------------------------------
...
_scroll_text:
	push	af
	dec	sp
;thumby.c:533: for (counter = 0; counter < len; ++counter) {
	ld	b,#0x00
00103$:
	ld	a,b
	ldhl	sp,#5
	sub	a, (hl)
	jp	NC,00105$
;thumby.c:534: x = 2 + (counter % 16);
	ld	a,b
	and	a, #0x0F
	add	a, #0x02
	ldhl	sp,#0
	ld	(hl),a
;thumby.c:535: y = 2 + divide_ubyte(counter, 16);
	push	bc
	ld	a,#0x10
	push	af
	inc	sp
	...
	inc	c
	inc	c
;thumby.c:539: set_win_tiles(x, y, 1, 1, &text_map[counter]);
	ld	de,#_text_map
	ld	l,b
	ld	h,#0x00
	add	hl,de
...
00105$:
	...
	ret
_scroll_text_end::
;thumby.c:544: void sleep(UBYTE cycles)
;	---------------------------------
; Function sleep
; ---------------------------------
_sleep_start::
_sleep:
;thumby.c:547: for(counter = 0; counter < cycles; counter++){
	ld	b,#0x00
00103$:
	ld	a,b
	ldhl	sp,#2
...
;thumby.c:547: for(counter = 0; counter < cycles; counter++){
	...
	ret
_sleep_end::
;thumby.c:552: void process_button_press(UBYTE *x, UBYTE *y)
;	---------------------------------
...
_process_button_press:
	add	sp, #-6
;thumby.c:554: UBYTE input = joypad();
	call	_joypad
;thumby.c:555: UBYTE move = 1;
	ldhl	sp,#0
	ld	(hl),#0x01
;thumby.c:558: if(input & J_A)
	ldhl	sp,#4
	ld	(hl),e
	inc	hl
	ld	(hl),#0x00
...
;thumby.c:581: void set_girl_sprite()
;	---------------------------------
; Function set_girl_sprite
; ---------------------------------
_set_girl_sprite_start::
_set_girl_sprite:
;thumby.c:584: for (counter = 0; counter < 8; ++counter) {
	ld	b,#0x00
00102$:
;thumby.c:587: counter++;
	inc	b
;thumby.c:588: counter--;
	dec	b
;thumby.c:589: set_sprite_tile(counter, counter*2);
	ld	a,b
	add	a, a
	push	bc
	push	af
...
;thumby.c:594: void update_girl_sprite(UBYTE x, UBYTE y)
;	---------------------------------
; Function update_girl_sprite
; ---------------------------------
_update_girl_sprite_start::
_update_girl_sprite:
;thumby.c:597: move_sprite(0,x+(0%4)*8,y+(0/4)*16);
	ldhl	sp,#3
	ld	a,(hl)
	push	af
	inc	sp
	...
	call	_move_sprite
	add	sp, #3
;thumby.c:598: move_sprite(1,x+(1%4)*8,y+0*16);
	ldhl	sp,#2
	ld	a,(hl)
	add	a, #0x08
	ld	d,a
	...
	add	sp, #3
	pop	de
;thumby.c:599: move_sprite(2,x+(0%4)*8,y+(1)*16);
	ldhl	sp,#3
	ld	a,(hl)
	add	a, #0x10
	ld	b,a
	...
	pop	de
	pop	bc
;thumby.c:600: move_sprite(3,x+(1%4)*8,y+(1)*16);
	push	bc
	push	bc
	inc	sp
	push	de
	...
	add	sp, #3
	pop	bc
;thumby.c:602: move_sprite(4,x+(2%4)*8,y+(0/4)*16);
	ldhl	sp,#2
	ld	a,(hl)
	add	a, #0x10
	ld	c,a
	...
	add	sp, #3
	pop	bc
;thumby.c:603: move_sprite(5,x+(3%4)*8,y+0*16);
	ldhl	sp,#2
	ld	a,(hl)
	add	a, #0x18
	ld	d,a
	...
	pop	de
	pop	bc
;thumby.c:604: move_sprite(6,x+(2%4)*8,y+(1)*16);
	push	bc
	push	de
	push	bc
	inc	sp
	...
	pop	de
	pop	bc
;thumby.c:605: move_sprite(7,x+(3%4)*8,y+(1)*16);
	push	bc
	inc	sp
	push	de
	inc	sp
...
;thumby.c:609: void check_level_one()
;	---------------------------------
...
_check_level_one:
	add	sp, #-29
;thumby.c:611: UBYTE counter, result = 1;
	ldhl	sp,#25
	ld	(hl),#0x01
;thumby.c:612: UBYTE tile_data[24] = {0};
	ldhl	sp,#0
	ld	a,l
	ld	d,h
	ldhl	sp,#26
	...
	xor	a, a
	ld	(bc),a
;thumby.c:615: for (counter = 0; counter < 8; ++counter) {
	ldhl	sp,#24
	ld	(hl),#0x00
00106$:
;thumby.c:616: result &= (get_sprite_tile(counter) == (0x10U + counter));
	ldhl	sp,#24
	ld	a,(hl)
	push	af
	inc	sp
...
;thumby.c:615: for (counter = 0; counter < 8; ++counter) {
	...
	sub	a, #0x08
	jr	C,00106$
;thumby.c:620: if (result == 1) {
	inc	hl
	ld	a,(hl)
	dec	a
	jr	NZ,00103$
;thumby.c:621: pass_level_one();
	call	_pass_level_one
00103$:
;thumby.c:625: get_sprite_data(6, 2, tile_data);
	ldhl	sp,#27
	dec	hl
	ld	e,(hl)
	inc	hl
	...
	add	sp, #4
;thumby.c:627: result = (memcmp(tile_data, &(girl_tiles[16*3]), 16) == 0);
	ldhl	sp,#27
	dec	hl
	ld	e,(hl)
	inc	hl
...
00128$:
	ldhl	sp,#25
	ld	(hl),a
;thumby.c:632: if (result == 1) {
	ld	a,(hl)
	dec	a
	jr	NZ,00108$
;thumby.c:633: pass_level_one();
	call	_pass_level_one
00108$:
	add	sp,#29
	ret
_check_level_one_end::
;thumby.c:639: void pass_level_one()
;	---------------------------------
; Function pass_level_one
; ---------------------------------
_pass_level_one_start::
_pass_level_one:
;thumby.c:641: SHOW_WIN;
	ld	de,#0xFF40
	ld	a,(de)
	ld	e,a
	ld	d,#0x00
	...
	ld	hl,#0xFF40
	ld	(hl),b
;thumby.c:643: show_text("DO YOU SEE MY   NEW OUTFIT?");
	ld	de,#__str_0
	push	de
	call	_show_text
	pop	af
;thumby.c:644: waitpad(J_A);
	...
	call	_waitpad
	inc	sp
;thumby.c:646: show_text("IS IT NOT CUTE?");
	ld	de,#__str_1
	push	de
	call	_show_text
	pop	af
;thumby.c:647: waitpad(J_A);
	ld	a,#0x10
	push	af
	inc	sp
	call	_waitpad
	inc	sp
;thumby.c:650: show_text("I WONDER IF IT  ENCODES...");
	ld	de,#__str_2
	push	de
	call	_show_text
	pop	af
;thumby.c:651: waitpad(J_A);
	ld	a,#0x10
	push	af
	inc	sp
	call	_waitpad
	inc	sp
;thumby.c:652: show_text("A SECRET FLAG!");
	ld	de,#__str_3
	push	de
	call	_show_text
	pop	af
;thumby.c:653: waitpad(J_A);
	ld	a,#0x10
	push	af
	inc	sp
	call	_waitpad
	inc	sp
;thumby.c:654: HIDE_WIN;
	ld	de,#0xFF40
	ld	a,(de)
	ld	b,a
	ld	d,#0x00
...
__str_3:
	.ascii "A SECRET FLAG!"
	.db 0x00
;thumby.c:660: void show_text(char *text)
;	---------------------------------
; Function show_text
; ---------------------------------
_show_text_start::
_show_text:
	push	af
	push	af
;thumby.c:663: UBYTE cursor_state = 0;
	ldhl	sp,#1
	ld	(hl),#0x00
;thumby.c:664: UBYTE text_len = strlen(text);
	ldhl	sp,#6
	ld	a,(hl+)
	ld	h,(hl)
	ld	l,a
	...
	pop	af
	ld	b,e
;thumby.c:666: clear_window();
	push	bc
	call	_clear_window
	pop	bc
;thumby.c:667: set_text_map(text, text_len);
	push	bc
	push	bc
	inc	sp
	ldhl	sp,#9
	...
	add	sp, #3
	pop	bc
;thumby.c:670: text_len += 1;
	inc	b
;thumby.c:671: scroll_text(text_len);
	push	bc
	push	bc
	inc	sp
	call	_scroll_text
	inc	sp
	pop	bc
;thumby.c:675: x = 1 + (text_len % 16);
	ld	a,b
	and	a, #0x0F
	ld	c,a
	inc	c
;thumby.c:676: y = 1 + divide_ubyte(text_len, 16);
	push	bc
	ld	a,#0x10
	push	af
	inc	sp
...
00104$:
	...
	or	a,d
	jp	Z,00107$
;thumby.c:679: if (cursor_state == 0) {
	ldhl	sp,#1
	ld	a,(hl)
	or	a, a
	jr	NZ,00102$
;thumby.c:680: text_map[text_len] = arrow_address_2;
	inc	hl
	ld	a,(hl+)
	ld	h,(hl)
	ld	l,a
	...
	jr	00103$
00102$:
;thumby.c:682: text_map[text_len] = arrow_address_1;
	ldhl	sp,#2
	ld	a,(hl+)
	ld	h,(hl)
	ld	l,a
	ld	(hl),#0x34
00103$:
;thumby.c:685: set_win_tiles(x, y, 1, 1, &text_map[text_len]);
	ldhl	sp,#3
	dec	hl
	ld	e,(hl)
	inc	hl
	...
	add	sp, #6
	pop	bc
;thumby.c:686: cursor_state = !cursor_state;
	ldhl	sp,#1
	ld	a,(hl)
	sub	a,#0x01
	ld	a,#0x00
...
;thumby.c:695: y=40;
	inc	hl
	ld	(hl),#0x28
;thumby.c:697: disable_interrupts();
	call	_disable_interrupts
;thumby.c:698: DISPLAY_OFF;
	call	_display_off
;thumby.c:701: set_bkg_data(0, grass_tiles_len, grass_tiles);
	ld	de,#_grass_tiles
	push	de
	ld	hl,#0x0500
	push	hl
	call	_set_bkg_data
	add	sp, #4
;thumby.c:702: set_bkg_tiles(0, 0, grass_field_width, grass_field_height, grass_field);
	ld	de,#_grass_field
	push	de
	ld	hl,#0x2020
	push	hl
...
;thumby.c:705: SPRITES_8x16;
	...
	ld	hl,#0xFF40
	ld	(hl),b
;thumby.c:706: set_sprite_data(0, girl_pj_tiles_len, girl_pj_tiles);
	...
	add	sp, #4
;thumby.c:707: set_sprite_data(16, girl_tiles_len, girl_tiles);
	ld	de,#_girl_tiles
	push	de
	ld	hl,#0x1010
	push	hl
	call	_set_sprite_data
	add	sp, #4
;thumby.c:708: set_girl_sprite();
	call	_set_girl_sprite
;thumby.c:709: update_girl_sprite(x, y);
	ld	hl,#0x2828
	push	hl
	call	_update_girl_sprite
	pop	af
;thumby.c:712: set_win_data(0x80, window_border_len, window_border);
	...
	call	_set_win_data
	add	sp, #4
;thumby.c:713: set_win_tiles(0, 0, text_box_width, text_box_height, text_box);
	ld	de,#_text_box
	push	de
	ld	hl,#0x0614
	push	hl
	...
	call	_set_win_tiles
	add	sp, #6
;thumby.c:714: move_win(7,96);
	ld	hl,#0x6007
	push	hl
	call	_move_win
	pop	af
;thumby.c:717: set_win_data(5U, alphabet_len, alphabet);
	ld	de,#_alphabet
	push	de
	ld	hl,#0x2F05
	push	hl
	call	_set_win_data
	add	sp, #4
;thumby.c:718: set_win_tiles(2U, 2U, text_map_width, text_map_height, text_map);
	ld	de,#_text_map
	push	de
	ld	hl,#0x0210
	push	hl
	...
	call	_set_win_tiles
	add	sp, #6
;thumby.c:721: set_win_data(arrow_address_1, arrow_tile_len, arrow_tile);
	ld	de,#_arrow_tile
	push	de
	ld	hl,#0x0234
	push	hl
	call	_set_win_data
	add	sp, #4
;thumby.c:725: set_sprite_data(32, 10, python_puzzle); // starting at tile 32 for 10 tiles
...
;thumby.c:729: DISPLAY_ON;
	...
	ld	l, #0x40
	ld	(hl),b
;thumby.c:730: enable_interrupts();
	call	_enable_interrupts
;thumby.c:735: SHOW_WIN;
	ld	de,#0xFF40
	ld	a,(de)
	ld	e,a
	ld	d,#0x00
	...
	ld	hl,#0xFF40
	ld	(hl),b
;thumby.c:736: show_text("MORNING...");
	ld	de,#__str_4
	push	de
	call	_show_text
	pop	af
;thumby.c:737: show_text("HELP ME GET DRESSED.");
	ld	de,#__str_5
	push	de
	call	_show_text
	pop	af
;thumby.c:738: show_text("MY CLOTHES ARE  IN VRAM.");
	ld	de,#__str_6
	push	de
	call	_show_text
	pop	af
;thumby.c:739: HIDE_WIN;
	ld	de,#0xFF40
	ld	a,(de)
	ld	b,a
	ld	d,#0x00
	...
	ld	hl,#0xFF40
	ld	(hl),b
;thumby.c:741: while(1) {
00103$:
;thumby.c:743: for(counter = 0; counter < 4; counter++){
	ld	b,#0x04
00107$:
;thumby.c:744: wait_vbl_done();
	push	bc
	call	_wait_vbl_done
	pop	bc
	ld	a,b
...
;thumby.c:743: for(counter = 0; counter < 4; counter++){
	...
	or	a, a
	jr	NZ,00107$
;thumby.c:746: process_button_press(&x, &y);
	ldhl	sp,#1
	ld	c,l
	ld	b,h
	ldhl	sp,#0
	...
	add	sp, #4
;thumby.c:747: update_girl_sprite(x, y);
	ldhl	sp,#1
	ld	a,(hl)
	push	af
	inc	sp
